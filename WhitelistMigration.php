...
class WhitelistMigration {
	...
	const TYPE_NUMERIC = ["TINYINT", "SMALLINT", "MEDIUMINT", "INT", "BIGINT", "DECIMAL", "FLOAT", "DOUBLE", "REAL", "BIT", "BOOLEAN"];
	 ...
	 */
	function __construct(MigrateInterface $mig) 
	{
		$this->mig = $mig;
	...
	public function columns(array $columns): bool 
	{
		foreach($columns as $name) {
			if(($p = strpos($name, ".")) !== false) $name = substr($name, $p+1);
			if($name !== "*") {
	...
	public function where(string $key, AttrInterface $value): bool 
	{
		...
		if(($p = strpos($key, ".")) !== false) $key = substr($key, $p+1);
		...
		if(DB::hasVariable($key)) $value = (string)DB::getVariableValue($key);
